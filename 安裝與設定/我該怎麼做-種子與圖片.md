...
若首頁服務點「加入購物車」時出現「此服務尚未設定規格」，請再執行一次規格種子：  
`psql "Zeabur 的 DATABASE_URL" -f backend/prisma/seed-data-service-variants.sql`  
（Zeabur 後端服務的 **Execute Command** 或本機用該連線字串執行皆可。）執行後 20 個服務都會有尺寸／顏色／部位／設計費可選。
