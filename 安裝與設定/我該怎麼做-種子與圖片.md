...
所以 Zeabur 現在：**登入、刺青師頭像、刺青分類** 都已經有了，你**不用再多做什麼**，重新整理網頁看就好。
...
5. 完成。不用再跑 seed-data-artist-photos.sql（因為現在的 seed-data.sql 已經有頭像了）。
...
- **以後新環境**：照「二」的順序，先 `seed-data.sql`，再視需要 `seed-data-homepage.sql`。
